/**
 * Derive KPV packshot from SEMAX master by replacing product name.
 * Similar to derive-ss31-packshot.mjs but for KPV peptide.
 */
...
const masterPath = path.join(productsDir, "semax-10mg.png");
const outPath = path.join(productsDir, "kpv-10mg.png");

/**
 * Locate the product-name line on a short-code vial
 * (name above dosage, below the hex logo).
 */
...
console.log(
  `SEMAX → KPV packshot ("${label}", font ${fontSize}px, name ${region.textWidth}x${region.textHeight} at ${region.textLeft},${region.textTop})`,
);
