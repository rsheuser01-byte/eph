/**
 * ONE-OFF: restamp the MT-2 10mg vial photo with an SS-31 name.
 * Not a build step. Prefer `node scripts/enlarge-vial-names.mjs` which
 * also sizes PT-141 to the same photographed short-code height.
 */
...
const outPath = path.join(productsDir, "ss-31-10mg.png");

/**
 * Locate the product-name line on a hyphenated short-code vial
 * (same layout as MT-2: name above dosage, below the hex logo).
 */
async function findNameRegion(filePath, dosage) {
  ...
  const y0 = Math.floor(height * 0.58);
...
while (probeY > region.textTop - 24 && probeY > 0) {
  let ink = 0;
  for (let x = x0; x <= x1; x++) {
    const i = (probeY * region.canvasWidth + x) * region.channels;
...
console.log(
  `MT-2 → SS-31 packshot ("${label}", font ${fontSize}px, name ${region.textWidth}x${region.textHeight} at ${region.textLeft},${region.textTop})`,
);
